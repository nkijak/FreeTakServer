...
logger = CreateLoggerController(
    "FTS_ClientReceptionHandler", logging_constants=loggingConstants
).getLogger()
...
class ClientReceptionHandler:
    ...
    def monitorForData(self, queue):
        ...
        try:
            keys = copy.deepcopy(
                list(self.clientInformationArray.keys())
            )  # this prevents changes to the clientInformationArray from having any severe effects on this method
            for user_id in keys:
                ...
                try:
                    ...
                    try:
                        ...
                        if xmlstring == b"" or xmlstring == "" or xmlstring is None:
                            ...
                            continue
                        xmlstring = (
                            "<multiEvent>" + xmlstring + "</multiEvent>"
                        ...
                        for event in events.findall("event"):
                            self.returnReceivedData(
                                client, etree.tostring(event), queue
                            )  # send each instance of event to the core
                        ...
                        ):  # this prevents errno 11 from spontanieously disconnecting clients due to the socket blocking set to 0
                    ...
                    logger.error(
                        loggingConstants.CLIENTRECEPTIONHANDLERMONITORFORDATAERRORD
                        + str(e)
                    )
                    self.returnReceivedData(client, b"", queue)
    ...
    def returnReceivedData(self, clientInformation, data, queue):
        try:
            ...
            RawCoT.xmlString = data.replace(
                b"\n", b""
            )  # replace all newlines with empty
            self.dataPipe.append(RawCoT)
            logger.debug("data received " + str(data))
            return 1
        except Exception as e:
            logger.error(
                loggingConstants.CLIENTRECEPTIONHANDLERRETURNRECEIVEDDATAERROR + str(e)
            )
            return -1
