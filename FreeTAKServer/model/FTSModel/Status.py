...
#
#######################################################
from FreeTAKServer.model.FTSModelVariables.statusVariables import (
    statusVariables as vars,
)
...
class Status(FTSProtocolObject):
    ...
    @staticmethod
    def drop_point(
        battery=vars.drop_point().BATTERY, readiness=vars.drop_point().READINESS
    ):
        status = Status()
    ...
    @staticmethod
    def connection(
        battery=vars.connection().BATTERY, readiness=vars.connection().READINESS
    ):
        status = Status()
        status.setbattery(battery=battery)
        status.setreadiness(readiness=readiness)
