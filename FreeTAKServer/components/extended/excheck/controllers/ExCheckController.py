...
logger = CreateLoggerController(
    "ExCheckController", logging_constants=loggingConstants
).getLogger()
...
class ExCheckController:
    ...
    def template(self, pipe):
        try:
            ...
            path = str(
                PurePath(Path(config.ExCheckFilePath), Path(f"{object.data.uid}.xml"))
            )
            ...
            resources = (
                ...
                .find("MissionChanges")
                .find("MissionChange")
                .find("contentResource")
            )
            resources.find("filename").text = (
                temp.find("checklistDetails").find("uid").text + ".xml"
            )
            resources.findall("keywords")[0].text = (
                temp.find("checklistDetails").find("name").text
            )
            resources.findall("keywords")[1].text = (
                temp.find("checklistDetails").find("description").text
            )
            resources.findall("keywords")[2].text = (
                temp.find("checklistDetails").find("creatorCallsign").text
            )
    ...
    def startList(self, subscription):
        ...
        with open(
            str(PurePath(Path(config.ExCheckChecklistFilePath), Path(f"{uid}.xml"))),
            "w+",
        ) as file:
            file.write(
                str(
                    open(
                        str(
                            PurePath(
                                Path(config.ExCheckFilePath),
                                Path(f"{subscription}.xml"),
                            )
                        ),
                        "r",
                    ).read()
        ...
        xml = etree.parse(
            config.ExCheckChecklistFilePath + "/" + uid + ".xml"
        ).getroot()
        ...
        excheckobj = self.dbController.query_ExCheck(
            f'ExCheckData.uid = "{subscription}"', verbose=True
        )[0]
        ...
        return (
            str(
                open(
                    str(
                        PurePath(
                            Path(config.ExCheckChecklistFilePath), Path(uid + ".xml")
                        )
                    ),
                    "r",
                ).read()
    ...
    def accesschecklist(self, checklistid):
        ...
        return str(
            open(
                str(
                    PurePath(
                        Path(config.ExCheckChecklistFilePath),
                        Path(checklistid + ".xml"),
                    )
                ),
                "r",
            ).read()
    ...
    def updatetemplate(self, checklistid, taskid, pipe):
        ...
        xml = etree.parse(
            str(
                PurePath(
                    Path(config.ExCheckChecklistFilePath), Path(checklistid + ".xml")
                )
            )
        ).getroot()
        ...
        object.detail.mission.MissionChanges.MissionChange.contentResource.hash.setINTAG(
            str(
                hashlib.sha256(
                    str(
                        open(
                            config.ExCheckChecklistFilePath
                            + "/"
                            + checklistid
                            + ".xml",
                            "r",
            ...
            if request.method == "GET":
                ...
                for checklist in excheckChecklists:
                    ...
                    checklistjson = {
                        ...
                        "startTime": datetime.strftime(
                            checklist.startTime, "%Y-%m-%dT%H:%M:%S.%fZ"
                        ),
                ...
                for item in ExCheckArray["Templates"]:
                    templateitem = DatabaseController().query_ExCheck(
                        f'ExCheckData.uid = "{item["uid"]}"', verbose=True
                    )[0]
                    os.remove(
                        str(
                            PurePath(
                ...
                try:
                    ...
                    path = str(
                        PurePath(
                            Path(config.ExCheckFilePath), Path(f"{object.data.uid}.xml")
                        )
                    )
                    with open(path, "w+") as file:
                        file.write(XMI)
                    ...
                    resources.find("uid").text = (
                        temp.find("checklistDetails").find("uid").text
                    )
                    resources.find("name").text = (
                        temp.find("checklistDetails").find("uid").text
                    )
                    resources.find("size").text = str(len(XMI))
                    resources.find("hash").text = str(
