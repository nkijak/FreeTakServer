...
class TableController:
    ...
    def query_by(self, session, columns, **kwargs):
        output = (
            session.query(
                *tuple(
                    [
                        getattr(self.table, x) if x != "*" else self.table
                        for x in columns
                    ]
                )
            )
    ...
    def update(self, session, query, column_value):
        DataPackages = (
            session.query(self.table).filter(text(query)).all()
        )  # self.query(session, query, [column for column, value in column_value.items()])
