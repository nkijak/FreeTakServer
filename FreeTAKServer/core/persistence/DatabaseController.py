...
from FreeTAKServer.components.extended.excheck.persistence import (
    ExCheckChecklistController,
)
from FreeTAKServer.core.persistence import APICallController
from FreeTAKServer.core.persistence import (
    _VideoTableController,
    ActiveEmergencysController,
)
...
class DatabaseController:
    ...
    def __init__(self):
        ...
        self.DataPackageController = (
            DataPackageTableController.DataPackageTableController()
        )
        self.UserTableController = UserTableController.UserTableController()
        self.SystemUserTableController = (
            system_user_table_controller.SystemUserTableController()
        )
        self.VideoStreamTableController = (
            VideoStreamTableController.VideoStreamTableController()
        )
        ...
        self.ActiveEmergencysController = (
            ActiveEmergencysController.ActiveEmergencysController()
        )
        self.ExCheckController = ExCheckController.ExCheckController()
        self.ExCheckChecklistController = (
            ExCheckChecklistController.ExCheckChecklistController()
        )
        self.ActiveFederationController = (
            ActiveFederationsController.ActiveFederationsController()
        )
    ...
    def query_datapackage(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.DataPackageController, query=query, columns=column
        )

    def update_datapackage(self, column_value=None, query="1=1"):
        return self._update(
            controller=self.DataPackageController,
            query=query,
    ...
    def _update(self, controller, column_value, query):
        return controller.update(
            session=self.session, column_value=column_value, query=query
        )
    ...
    def create_user(self, **args):
        try:
            ...
            if (
                str(e)
                == """(sqlite3.IntegrityError) UNIQUE constraint failed: User.uid
...
(Background on this error at: http://sqlalche.me/e/13/gkpj)"""
    ...
    def query_user(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.UserTableController, query=query, columns=column
        )

    def update_user(self, column_value=dict(), query="1=1"):
        return self._update(
            controller=self.UserTableController, query=query, column_value=column_value
        )
    ...
    def query_by_systemUser(self, columns=["*"], **kwargs):
        return self._query_by(
            controller=self.SystemUserTableController, columns=columns, **kwargs
        )

    def query_systemUser(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.SystemUserTableController, query=query, columns=column
        )

    def update_systemUser(self, column_value=dict, query="1=1"):
        return self._update(
            controller=self.SystemUserTableController,
            query=query,
            column_value=column_value,
        )
    ...
    def query_videostream(self, query="1=1", column=["*"]):
        return self._query(
            controller=self._VideoTableController, query=query, columns=column
        )

    def update_videostream(self, column_value=dict(), query="1=1"):
        return self._update(
            controller=self._VideoTableController,
            query=query,
            column_value=column_value,
        )
    ...
    def update_video(self, column_value=dict(), query="1=1"):
        return self._update(
            controller=self.VideoStreamTableController,
            query=query,
            column_value=column_value,
        )
    ...
    def query_CoT(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.EventTableController, query=query, columns=column
        )

    def update_CoT(self, column_value=dict(), query="1=1"):
        return self._update(
            controller=self.EventTableController, query=query, column_value=column_value
        )
    ...
    def query_APIUser(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.APIUserController, query=query, columns=column
        )

    def update_APIUser(self, column_value=None, query="1=1"):
        return self._update(
            controller=self.APIUserController, query=query, column_value=column_value
        )
    ...
    def query_ActiveEmergency(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.ActiveEmergencysController, query=query, columns=column
        )

    def update_ActiveEmergency(self, column_value=None, query="1=1"):
        return self._update(
            controller=self.ActiveEmergencysController,
            query=query,
            column_value=column_value,
        )
    ...
    def query_ExCheck(self, query="1=1", column=["*"], verbose=False):
        if verbose == False:
            return self._query(
                controller=self.ExCheckController, query=query, columns=column
            )
        else:
    ...
    def update_ExCheck(self, query="1=1", column_value=None):
        return self._update(
            controller=self.ExCheckController, query=query, column_value=column_value
        )
    ...
    def query_ExCheckChecklist(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.ExCheckChecklistController, query=query, columns=column
        )

    def update_ExCheckChecklist(self, column_value=None, query="1=1"):
        return self._update(
            controller=self.ExCheckChecklistController,
            query=query,
            column_value=column_value,
        )
    ...
    def query_ActiveFederation(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.ActiveFederationController, query=query, columns=column
        )

    def update_ActiveFederation(self, column_value=None, query="1=1"):
        return self._update(
            controller=self.ActiveFederationController,
            query=query,
            column_value=column_value,
        )
    ...
    def query_Federation(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.FederationController, query=query, columns=column
        )

    def update_Federation(self, column_value=None, query="1=1"):
        return self._update(
            controller=self.FederationController, query=query, column_value=column_value
        )
    ...
    def query_APICall(self, query="1=1", column=["*"]):
        return self._query(
            controller=self.APICallController, query=query, columns=column
        )

    def update_APICall(self, column_value=None, query="1=1"):
        return self._update(
            controller=self.APICallController, query=query, column_value=column_value
        )
