...
class LoggingConstants:
    def __init__(self, log_name="FTS"):
        ...
        if os.path.isdir("/var/log") and os.access("/var/log", os.W_OK):
            ...
            Path(PurePath(self.PARENTPATH, self.LOGDIRECTORY)).mkdir(
        ...
        self.ERRORLOG = str(
            PurePath(self.PARENTPATH, f"{self.LOGDIRECTORY}/{self.LOGNAME}_error.log")
        )
        self.DEBUGLOG = str(
            PurePath(self.PARENTPATH, f"{self.LOGDIRECTORY}/{self.LOGNAME}_debug.log")
        )
        self.INFOLOG = str(
            PurePath(self.PARENTPATH, f"{self.LOGDIRECTORY}/{self.LOGNAME}_info.log")
        )
        self.HTTPLOG = str(
            PurePath(self.PARENTPATH, f"{self.LOGDIRECTORY}/{self.LOGNAME}_http.log")
        )
        self.DELIMITER = " ? "
        ...
        # dataReceived
        self.DATARECEIVEDERROR = (
            "there has been an error in the reception of generic data"
        )
        ...
        self.MONITORRAWCOTERRORINTERNALSCANERROR = (
            "there has been an error in the scanning of the internal CoT array"
        )
        ...
        self.ARGUMENTCOTPORTDESCRIPTION = (
            "the port you would like FreeTAKServer to run receive connections on"
        )
        ...
        self.ARGUMENTDATAPORTDESCRIPTION = (
            "the port you would like FreeTAKServer to run receive connections on"
        )
        ...
        # Startup
        self.CLIENTRECEPTIONHANDLERSTART = (
            "client reception handler has finished startup"
        )
        self.CLIENTRECEPTIONHANDLERSTARTUPERROR = (
            "there has been an error in client reception startup"
        )

        # Monitor event pipe
        self.CLIENTRECEPTIONHANDLERMONITOREVENTPIPEERROR = (
            "there has been an error in a client reception Event Pipe"
        )
        ...
        self.CLIENTRECEPTIONHANDLERCREATECLIENTMONITORINFO = (
            "client reception handler thread has finished being created"
        )

        # destroy client monitor
        self.CLIENTRECEPTIONHANDLERDESTROYCLIENTMONITORINFO = (
            "client reception handler thread has finished being terminated"
        )
        ...
        # monitorForData
        self.CLIENTRECEPTIONHANDLERMONITORFORDATAERRORA = (
            "there has been an error in a clients reception thread section A "
        )
        self.CLIENTRECEPTIONHANDLERMONITORFORDATAERRORB = (
            "there has been an error in a clients reception thread section B "
        )
        self.CLIENTRECEPTIONHANDLERMONITORFORDATAERRORC = (
            "there has been an error in a clients reception thread section C "
        )
        self.CLIENTRECEPTIONHANDLERMONITORFORDATAERRORD = (
            "there has been an error in a clients reception thread section D "
        )
        ...
        self.RECEIVECONNECTIONSLISTENERROR = (
            "error in Receive connections listen function "
        )
        ...
        # Send data in queue
        self.SENDDATACONTROLLERSENDDATAINQUEUEERROR = (
            "error send data controller in send data in queue"
        )

        # XMLCoTController
        # determine CoT general
        self.XMLCOTCONTROLLERDETERMINECOTGENERALERRORA = (
            "exception XMLCoTController in determining general CoT type A "
        )
        self.XMLCOTCONTROLLERDETERMINECOTGENERALERRORB = (
            "exception XMLCoTController in determining general CoT type B "
        )
        self.XMLCOTCONTROLLERDETERMINECOTGENERALERRORC = (
            "exception XMLCoTController in determining general CoT type C "
        )
