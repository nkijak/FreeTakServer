...
class AbstractApiAdapter(JsonSerializer):
    ...
    def _create_cot_object(
        self,
        serialized_json: dict,
        expected_fts_obj: Event,
        final_cot_object: Types.specific_cot,
    ) -> Types.specific_cot:
        fts_object = super().from_format_to_fts_object(
            serialized_json, expected_fts_obj
        )
        CoTObject = final_cot_object
        CoTObject.setModelObject(fts_object)
        CoTObject.setXmlString(
            etree.tostring(XmlSerializer().from_fts_object_to_format(fts_object))
        )
        return CoTObject
...
class GeoObjectAdapter(AbstractApiAdapter):
    def from_api_to_fts_object(self, api_json: dict) -> Types.specific_cot:
        ...
        CoTObject = self._create_cot_object(
            serialized_json, Event.SimpleCoT(), SendSimpleCoT()
        )
        return CoTObject
    ...
    def serialize_initial_json(self, api_json: dict) -> dict:
        ...
        if "-.-" in RestEnumerations.geoObject[api_json["geoObject"]]:
            api_json["type"] = RestEnumerations.geoObject[
                api_json["geoObject"]
            ].replace("-.-", RestEnumerations.attitude[api_json["attitude"]])
        else:
...
class ChatAdapter(AbstractApiAdapter):
    def from_api_to_fts_object(self, api_json: dict) -> Types.specific_cot:
        ...
        serialized_json = self.serialize_initial_json(api_json)
        CoTObject = self._create_cot_object(
            serialized_json, Event.GeoChat(), SendGeoChat()
        )
        return CoTObject
...
class PresenceAdapter(AbstractApiAdapter):
    def from_api_to_fts_object(self, api_json: dict) -> Types.specific_cot:
        ...
        serialized_json = self.serialize_initial_json(api_json)
        CoTObject = self._create_cot_object(
            serialized_json, Event.Presence(), Presence()
        )
        return CoTObject
...
class EmergencyOnAdapter(AbstractApiAdapter):
    def from_api_to_fts_object(self, api_json: dict) -> Types.specific_cot:
        ...
        serialized_json = self.serialize_initial_json(api_json)
        CoTObject = self._create_cot_object(
            serialized_json, Event.emergecyOn(), SendEmergency()
        )
        return CoTObject
    ...
    def serialize_initial_json(self, api_json):
        ...
        del api_json["name"]
        api_json["event"] = {
            "type": RestEnumerations.emergencyTypes[api_json["emergencyType"]]
        }
