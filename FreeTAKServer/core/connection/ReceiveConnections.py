...
from FreeTAKServer.core.configuration.ClientReceptionLoggingConstants import (
...
from FreeTAKServer.core.configuration.ReceiveConnectionsConstants import (
    ReceiveConnectionsConstants,
)
...
logger = CreateLoggerController(
    "FTS_ReceiveConnections", logging_constants=loggingConstants
).getLogger()
...
class ReceiveConnections:
    ...
    def receive_connection_data(self, client) -> Union[etree.Element, str]:
        ...
        """
        client.settimeout(
            int(ReceiveConnectionsConstants().RECEIVECONNECTIONDATATIMEOUT)
        )
        part = client.recv(1)
        if part == b"":
        ...
        if part.decode() + xmlstring == ReceiveConnectionsConstants().TESTDATA:
            return TEST_SUCCESS
        client.setblocking(True)
        client.settimeout(
            int(ReceiveConnectionsConstants().RECEIVECONNECTIONDATATIMEOUT)
        ...
        xmlstring = (
            "<multiEvent>" + part.decode() + xmlstring + "</multiEvent>"
        )  # convert to xmlstring wrapped by multiEvent tags
        xmlstring = re.sub(
            r"(?s)\<\?xml(.*)\?\>", "", xmlstring
        )  # replace xml definition tag with empty string as it breaks serilization
        ...
        try:
            ...
            try:
                ...
                if sslstatus == True:
                    client = asyncio.run(
                        asyncio.wait_for(
                            SSLSocketController().wrap_client_socket(client),
                            timeout=ReceiveConnectionsConstants().WRAP_SSL_TIMEOUT,
                        )
                    )
            ...
            if sslstatus == True:
                logger.info(
                    "client connected over ssl " + str(address) + " " + str(time.time())
                )
            ...
            try:
                ...
                try:
                    ...
                    client.close()
                    logger.warning(
                        "receiving connection data from client failed with exception "
                        + str(e)
                    )
                    return -1
            ...
            raw_connection_information = self.instantiate_client_object(
    ...
    def instantiate_client_object(self, address, client, events):
        ...
        raw_connection_information.xmlString = etree.tostring(
            events.findall("event")[0]
        ).decode("utf-8")
        return raw_connection_information
    ...
    def recv_until(self, client, delimiter) -> bytes:
        ...
        while (
            ...
            and time.time() - start_receive_time
            <= ReceiveConnectionsConstants().RECEIVECONNECTIONDATATIMEOUT
        ):
            try:
                message = message + client.recv(
                    ReceiveConnectionsConstants().CONNECTION_DATA_BUFFER
                )
            except:
                return message
        return message
