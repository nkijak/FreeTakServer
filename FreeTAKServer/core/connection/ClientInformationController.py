...
logger = CreateLoggerController(
    "FTS_ClientInformationController", logging_constants=loggingConstants
).getLogger()
...
class ClientInformationController(BasicModelInstantiate):
    ...
    def intstantiateClientInformationModelFromConnection(
        ...
        try:
            ...
            self.clientInformation.modelObject = (
            ...
            logger.debug(
                "error in client information controller "
                + str(e)
                + " "
                + str(rawClientInformation.xmlString)
            )
            return -1
