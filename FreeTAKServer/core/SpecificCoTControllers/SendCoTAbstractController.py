...
logger = CreateLoggerController(
    "FTS_SendCoTAbstract", logging_constants=loggingConstants
).getLogger()
...
class SendCoTAbstractController(ABC):
    ...
    def fill_object(self, object, tempObject, RawCoT, addToDB=config.SaveCoTToDB):
        try:
            ...
            if tempObject.type != "other":
                ...
                logger.error(
                    ...
                    + str(e)
                    + " please post this as an issue on the FTS git so we can document and fix it"
                )
                return 1
            else:
                logger.warning(
                    "there has been an exception in the creation of the model object "
                    + str(e)
                )
                return -1
        ...
        try:
            ...
            logger.warning(
                "there has been an exception in the creation of a xmlString " + str(e)
            )
        try:
            if addToDB == True:
            ...
            print(e)
            logger.warning(
                "there has been an exception in the creation of a database instance of this object "
                + str(e)
            )
            raise Exception(e)
    ...
    def create_model_object(self, tempObject, xmlString):
        ...
        """
        modelInstance = xml_serializer.XmlSerializer().from_format_to_fts_object(
            xmlString, tempObject
        )
    ...
    def create_xml_string(self, modelObject):
        ...
        try:
            ...
            logger.error(
                "there has been an exception in the creation of the xml string "
                + str(e)
            )
            return -1
