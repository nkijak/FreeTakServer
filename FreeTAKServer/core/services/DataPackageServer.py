...
from FreeTAKServer.core.configuration.DataPackageServerConstants import (
...
logger = CreateLoggerController(
    "FTS-DataPackage_Service", logging_constants=loggingConstants
).getLogger()
...
app.logger.removeHandler(
    default_handler
)  # pylint: disable=no-member; member does exist
formatter = logging.Formatter(log.LOGFORMAT)
file_handler = RotatingFileHandler(
    log.HTTPLOG, maxBytes=log.MAXFILESIZE, backupCount=log.BACKUPCOUNT
)
...
def get_all_video_links():
    ...
    try:
        ...
        app.logger.info(
            f"Found {len(feeds)} video feeds in {const.DATABASE}"
        )  # pylint: disable=no-member; member does exist
        if len(feeds) == 0:
...
def insert_video_link():
    try:
        ...
        for xml_feed in xml_root:
            ...
            if len(streams) > 0:
                app.logger.info(
                    f"Already received feed with UID={uid} (alias = {alias})"
                )  # pylint: disable=no-member; member does exist
                continue  # Ignore this feed if there are duplicates
            app.logger.info(
                f"Inserting video feed into database: {request.data.decode('utf-8')}"
            )  # pylint: disable=no-member; member does exist
            dbController.create_videostream(
                FullXmlString=ET.tostring(xml_feed),
                Protocol=protocol,
                Alias=alias,
                uid=uid,
                Address=address,
                ...
                IgnoreEmbeddedKlv=ignore_klv,
                PreferredMacAddress=preferred_mac,
                Path=path,
                Buffer=buf,
                Timeout=timeout,
...
def upload():
    from FreeTAKServer.model.ServiceObjects.SSLDataPackageVariables import (
        SSLDataPackageVariables,
    )
    ...
    callsign = str(
        FlaskFunctions().getSubmissionUser(creatorUid, dbController)
    )  # fetchone() gives a tuple, so only grab the first element
    FlaskFunctions().create_dp(
        dbController,
        uid=uid,
        Name=filename,
        Hash=file_hash,
...
def putDataPackageTool(hash):
    if request.data == b"private":
        dbController.update_datapackage(
            query=f'Hash = "{hash}"', column_value={"Privacy": 1}
        )
    return "Okay", 200
...
def specificPackage():
    ...
    if request.method == "GET" and request.args.get("uid") != None:
        ...
        for file in listdir(config.ExCheckFilePath):
            try:
                xml = etree.parse(
                    str(PurePath(Path(config.ExCheckFilePath), Path(file)))
                ).getroot()
        ...
        if os.path.exists(str(PurePath(Path(dp_directory), Path(hash)))):
            ...
            app.logger.debug(
                PurePath(Path(const.DATAPACKAGEFOLDER), Path(hash), Path(file_list[0]))
            )
            path = PurePath(dp_directory, str(hash), file_list[0])
            ...
            data = etree.parse(
                str(PurePath(Path(config.ExCheckFilePath), Path(obj[0].data.filename)))
            )
...
class FlaskFunctions:
            ...
            package_dict["results"].append(
                {
                    ...
                    "SubmissionDateTime": str(
                        i.SubmissionDateTime.strftime("%Y-%m-%dT%H:%M:%S.%fZ")
                    ),
    ...
    def startup(self, ip, port, pipe):
        try:
            ...
            wsgi.server(
                eventlet.listen((DataPackageServerConstants().IP, int(HTTPPORT))), app
            )

        except Exception as e:
            logger.error(
                "there has been an exception in Data Package service startup " + str(e)
            )
            return -1
