...
from FreeTAKServer.core.SpecificCoTControllers.SendCoTAbstractController import (
...
class HandlerBase(HandlerInterface):
    ...
    def callNextHandler(self, obj: FederationServiceBase, command):
        if self.nextHandler is None:
            raise Exception(
                "no further handlers, object not supported by responsibility chain"
            )
        else:
...
class DisconnectHandler(HandlerBase):
    ...
    def Handle(self, obj: FederationServiceBase, command):
        if (
            isinstance(obj, FederationServiceBase)
            and isinstance(command, tuple)
            and command[1] == "DELETE"
        ):
    ...
    def Handle(self, obj: FederationServiceBase, command: SpecificCoTAbstract) -> None:
        ...
        if isinstance(command.modelObject, SendOther):
            if command.modelObject.martiPresent:
                ...
                cot_dest_callsigns = [
                    dest.callsign for dest in command.modelObject.detail.marti.dest
                ]
                ...
                if bool(set(user_callsigns) & set(cot_dest_callsigns)):
                    ...
                    raise Exception(
                        "this service has none of the desired clients for this CoT"
                    )

        elif hasattr(command.modelObject.detail, "marti") and hasattr(
            command.modelObject.detail.marti.dest[0], "callsign"
        ):
            user_callsigns = [user.contact.callsign for user in user_list]
            cot_dest_callsigns = [
                dest.callsign for dest in command.modelObject.detail.marti.dest
            ...
            if bool(
                set(user_callsigns) & set(cot_dest_callsigns)
            ) or cot_dest_callsigns == [None]:
                self.callNextHandler(obj, command)
            else:
                raise Exception(
                    "this service has none of the desired clients for this CoT"
...
class SendDataHandler(HandlerBase):
    ...
    def Handle(self, obj: FederationServiceBase, command: any):
        ...
        if isinstance(obj, FederationServiceBase) and isinstance(
            command, SpecificCoTAbstract
        ):
