...
from FreeTAKServer.core.services.service_abstracts import (
...
class FederationServiceBase(ServerServiceInterface, ServiceBase):
    ...
    def _process_protobuff_to_object(self, protobuf_object: FederatedEvent):
        ...
        """
        model_object, fts_object = XMLCoTController().determine_model_object_type(
            protobuf_object.event.type
        )  # pylint: disable=no-member; member does exist
        fts_object = fts_object()
        model_object = ProtobufSerializer().from_format_to_fts_object(
            protobuf_object, model_object()
        )
        ...
        try:
            if self.federates:
                ...
                if detail:
                    ...
                    try:
                        protobuf.event.other = etree.tostring(
                            detail
                        )  # pylint: disable=no-member; member does exist
                        ...
                        self.logger.warning(
                            "creating protobuf message failed " + str(e)
                        )
                        return None
    ...
    def send_connection_data(self, CoT: ClientInformation) -> None:
        try:
            if self.federates:
                ...
                proto_obj.contact.uid = str(
                    CoT.modelObject.uid
                )  # pylint: disable=no-member; member does exist
                proto_obj.contact.callsign = str(
                    CoT.modelObject.detail.contact.callsign
                )  # pylint: disable=no-member; member does exist
                proto_obj.contact.operation = (
                    1  # pylint: disable=no-member; member does exist
                )
            ...
            self.logger.warning(
                "exception throw sending new connection data to federates " + str(e)
            )
            return None
    ...
    def _send_connected_clients(self, connection):
        try:
            ...
            self.logger.warning(
                "error thrown in getting clients from DataBase to send to federates "
                + str(e)
            )
            return None
    ...
    def disconnect_client(self, id: str) -> None:
        try:
            ...
            try:
                ...
                self.logger.warning(
                    "federate array has no item with uid "
                    + str(id)
                    + " federates array is len "
            ...
            self.logger.warning(
                "exception thrown accessing client for disconnecting client " + str(e)
            )
    ...
    def send_disconnection_data(self, CoT: SendDisconnect) -> None:
        if self.federates:
            proto_obj = FederatedEvent()
            proto_obj.contact.uid = str(
                CoT.modelObject.detail.link.uid
            )  # pylint: disable=no-member; member does exist
            proto_obj.contact.callsign = str(
                CoT.modelObject.detail.link.type
            )  # pylint: disable=no-member; member does exist
            proto_obj.contact.operation = (
                4  # pylint: disable=no-member; member does exist
            )
            proto_str = proto_obj.SerializeToString()
            header = self._generate_header(len(proto_str))
            for fed in self.federates.values():
